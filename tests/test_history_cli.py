...
def test_history_cli_lists_and_retrieves_revisions(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    listed = runner.invoke(app, ["history", bundle.incident.incident_id, "--workspace", str(tmp_path)])
    ...
    assert revision["payload"]["incident"]["incident_id"] == bundle.incident.incident_id


def test_history_cli_rejects_invalid_incident_id(tmp_path: Path) -> None:
    result = CliRunner().invoke(app, ["history", "invalid", "--workspace", str(tmp_path)])
